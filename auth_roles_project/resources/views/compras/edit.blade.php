...
<div style="max-width:640px;margin:0 auto">
    ...
    <h1 style="font-size:26px;font-weight:700;color:#0f172a;letter-spacing:-0.5px;margin:0 0 24px 0">Editar Compra #{{ $compra->Id_Com }}</h1>

    <div class="card">
        <form action="{{ route('compras.update', $compra->Id_Com) }}" method="POST" style="display:flex;flex-direction:column;gap:20px">
            @csrf @method('PATCH')
            @if ($errors->any())
            ...
            <div>
                ...
                <select name="Id_Prod_FK" required class="input @error('Id_Prod_FK') is-invalid @enderror" id="producto-select">
                    @foreach ($productos as $producto)
                        <option value="{{ $producto->Id_pro }}" data-precio="{{ $producto->Precio_pro }}" {{ old('Id_Prod_FK', $compra->Id_Prod_FK) == $producto->Id_pro ? 'selected' : '' }}>{{ $producto->Nom_pro }} — ${{ number_format($producto->Precio_pro, 2) }}</option>
                    @endforeach
                </select>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
                <div>
                    ...
                    <input type="number" name="Cant_Com" id="cantidad-input" value="{{ old('Cant_Com', $compra->Cant_Com) }}" required min="1" class="input @error('Cant_Com') is-invalid @enderror">
                    @error('Cant_Com')<p class="input-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="input-label">Fecha de Compra</label>
                    <input type="date" name="Fecha_Com" value="{{ old('Fecha_Com', $compra->Fecha_Com ? \Carbon\Carbon::parse($compra->Fecha_Com)->format('Y-m-d') : date('Y-m-d')) }}" required class="input @error('Fecha_Com') is-invalid @enderror">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
                <div>
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="0.01" name="Precio_Com" id="precio-compra" value="{{ old('Precio_Com', $compra->Valor_Com > 0 && $compra->Cant_Com > 0 ? $compra->Valor_Com / $compra->Cant_Com : 0) }}" required min="0" class="input @error('Precio_Com') is-invalid @enderror" style="padding-left:32px">
                    </div>
            ...
            <div>
                ...
                <select name="Id_Proveedor" class="input @error('Id_Proveedor') is-invalid @enderror">
                    ...
                    @foreach ($proveedores as $proveedor)
                        <option value="{{ $proveedor->Id_Prov }}" {{ old('Id_Proveedor', $compra->Id_Proveedor) == $proveedor->Id_Prov ? 'selected' : '' }}>{{ $proveedor->Nom_Prov }}</option>
                    @endforeach
                </select>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (cantInput.value && precInput.value) actualizar();
    });
</script>
@endpush
@endsection