...
<div style="max-width:640px;margin:0 auto">
    ...
    <h1 style="font-size:26px;font-weight:700;color:#0f172a;letter-spacing:-0.5px;margin:0 0 24px 0">Nueva Venta</h1>

    <div class="card">
        <form action="{{ route('ventas.store') }}" method="POST" style="display:flex;flex-direction:column;gap:20px">
            @csrf
            @if ($errors->any())
            ...
            <div>
                <label class="input-label">Cliente <span style="color:#94a3b8;font-weight:400">(opcional, para factura)</span></label>
                <select name="Id_Cli" class="input @error('Id_Cli') is-invalid @enderror">
                    <option value="">Sin cliente (venta directa)</option>
                    @foreach ($clientes as $cliente)
                        <option value="{{ $cliente->Id_Cli }}" {{ old('Id_Cli') == $cliente->Id_Cli ? 'selected' : '' }}>{{ $cliente->Nom_Cli }} — {{ $cliente->Email_Cli ?? '' }}</option>
                    @endforeach
                </select>
                @error('Id_Cli')<p class="input-error">{{ $message }}</p>@enderror
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
                <div>
                    <label class="input-label">Cantidad</label>
                    <input type="number" name="Cant_Ven" id="cantidad-input" value="{{ old('Cant_Ven') }}" required min="1" placeholder="1" class="input @error('Cant_Ven') is-invalid @enderror">
                    @error('Cant_Ven')<p class="input-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="input-label">Fecha y Hora</label>
                    <input type="datetime-local" name="Fecha_Ven" value="{{ old('Fecha_Ven', now()->format('Y-m-d\TH:i')) }}" required class="input @error('Fecha_Ven') is-invalid @enderror">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
                <div>
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" id="precio-display" readonly placeholder="0.00" class="input" style="padding-left:32px;background:#f1f5f9;cursor:default">
                    </div>
                </div>
                <div>
