...
<div style="max-width:640px;margin:0 auto">
    ...
    <h1 style="font-size:26px;font-weight:700;color:#0f172a;letter-spacing:-0.5px;margin:0 0 24px 0">Editar Registro de Inventario</h1>

    <div class="card">
        <form action="{{ route('inventarios.update', $inventario->Id_Inven) }}" method="POST" style="display:flex;flex-direction:column;gap:20px">
            @csrf @method('PATCH')
            @if ($errors->any())
                ...
                <div>
                    ...
                    <select name="Id_Producto" required class="input @error('Id_Producto') is-invalid @enderror">
                        ...
                        @foreach ($productos as $producto)
                            <option value="{{ $producto->Id_pro }}" {{ old('Id_Producto', $inventario->Id_Producto) == $producto->Id_pro ? 'selected' : '' }}>{{ $producto->Nom_pro }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="Id_Proveedor" class="input @error('Id_Proveedor') is-invalid @enderror">
                        ...
                        @foreach ($proveedores as $proveedor)
                            <option value="{{ $proveedor->Id_Prov }}" {{ old('Id_Proveedor', $inventario->Id_Proveedor) == $proveedor->Id_Prov ? 'selected' : '' }}>{{ $proveedor->Nom_Prov }}</option>
                        @endforeach
                    </select>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px">
                <div>
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="0.01" name="Precio_Com" value="{{ old('Precio_Com', $inventario->Precio_Com) }}" required min="0" class="input @error('Precio_Com') is-invalid @enderror" style="padding-left:32px">
                    </div>
                ...
                <div>
                    ...
                    <div class="input-group">
                        <span style="position:absolute;left:16px;top:50%;transform:translateY(-50%);color:#94a3b8;font-size:14px;font-weight:500;pointer-events:none;z-index:1">$</span>
                        <input type="number" step="0.01" name="Precio_Ven" value="{{ old('Precio_Ven', $inventario->Precio_Ven) }}" required min="0" class="input @error('Precio_Ven') is-invalid @enderror" style="padding-left:32px">
                    </div>
                    @error('Precio_Ven')<p class="input-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="input-label">Stock</label>
                    <input type="number" name="Stock" value="{{ old('Stock', $inventario->Stock) }}" required min="0" class="input @error('Stock') is-invalid @enderror">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
                <div>
                    ...
                    <input type="text" name="Categoria" value="{{ old('Categoria', $inventario->Categoria) }}" placeholder="Ej: Alimentos, Juguetes" class="input @error('Categoria') is-invalid @enderror">
                    @error('Categoria')<p class="input-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="input-label">Descripción</label>
                    <textarea name="Descripcion" rows="3" class="input @error('Descripcion') is-invalid @enderror">{{ old('Descripcion', $inventario->Descripcion) }}</textarea>
